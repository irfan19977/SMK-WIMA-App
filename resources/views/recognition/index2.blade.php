...
@section('title', 'Face Recognition - Face Scan')
                        ...
                        <div class="col-md-7">
                            <div class="card border-primary h-100">
                                <div class="card-header bg-primary text-white">
                                    <h5 class="card-title mb-0"><i class="fas fa-user"></i> Profil Pengguna</h5>
                                </div>
                                <div class="card-body text-center">
                                    <div id="userProfile" style="display:none;">
                                        {{-- User Photo --}}
                                        <div class="mb-4">
                                            <img id="userPhoto" class="img-thumbnail border-primary" 
                                        ...
                                        <div class="row">
                                            <div class="col-md-12">
                                                <h3 id="userName" class="text-primary mb-3"></h3>
                                            </div>
                                        </div>
                                        
                                        <div class="row">
                                            <div class="col-md-6">
                                                <p class="mb-2"><strong>NISN:</strong> <span id="userNisn"></span></p>
                                                <p class="mb-2"><strong>Jenis Kelamin:</strong> <span id="userGender"></span></p>
                                                <p class="mb-2"><strong>Tempat Lahir:</strong> <span id="userBirthPlace"></span></p>
                                            </div>
                                            <div class="col-md-6">
                                                <p class="mb-2"><strong>Tanggal Lahir:</strong> <span id="userBirthDate"></span></p>
                                                <p class="mb-2"><strong>Wajah Terdaftar:</strong> <span id="userRegistered"></span></p>
                                                <p class="mb-2"><strong>Alamat:</strong> <span id="userAddress"></span></p>
                                            </div>
                                        </div>
                                    </div>

                                    {{-- Default state when no user detected --}}
                                    <div id="noUserProfile">
                                        <div class="text-muted py-5">
                                            <i class="fas fa-user-circle fa-5x mb-3"></i>
                                            <h4>Belum ada pengguna terdeteksi</h4>
                                            <p>Lakukan scan wajah untuk menampilkan profil pengguna</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="col-md-5">
                            <div class="card border-success mb-3">
                                <div class="card-header bg-success text-white">
                                    <h6 class="card-title mb-0"><i class="fas fa-camera"></i> Face Scan</h6>
                                </div>
                                <div class="card-body text-center">
                                    ...
                                    {{-- Camera Controls --}}
                                    <div class="mt-3">
                                        <button id="startCamera" class="btn btn-success btn-sm me-1">
                                            <i class="fas fa-camera"></i> Start
                                        </button>
                                        <button id="stopCamera" class="btn btn-secondary btn-sm me-1" disabled>
                                            <i class="fas fa-stop"></i> Stop
                                        </button>
                                        <button id="scanFace" class="btn btn-primary btn-sm me-1" disabled>
                                            <i class="fas fa-search"></i> Scan
                                        </button>
                                        <button id="clearResult" class="btn btn-warning btn-sm">
                            ...
                            <div class="card border-info">
                                <div class="card-header bg-info text-white">
                                    <h6 class="card-title mb-0"><i class="fas fa-tachometer-alt"></i> Status Pengenalan</h6>
                                </div>
                                <div class="card-body">
                                    <div id="recognitionStatus" class="text-center mb-3">
                                        <div class="spinner-border text-info" role="status" style="display:none;">
                                            <span class="visually-hidden">Loading...</span>
                                        </div>
                                        <p class="text-muted mb-0">Klik "Start" untuk memulai</p>
                                    </div>
                                    <div id="recognitionStats" class="small">
                                        <div class="d-flex justify-content-between mb-1">
                                            <span>Wajah Terdaftar:</span>
                                            <span class="badge bg-info" id="registeredCount">{{ $registeredStudents->count() }}</span>
                                        </div>
                                        <div class="d-flex justify-content-between mb-1">
                        ...
                        <div class="col-md-12">
                            <div class="card border-secondary">
                                <div class="card-header bg-secondary text-white">
                                    <h5 class="card-title mb-0"><i class="fas fa-history"></i> Riwayat Absensi</h5>
                                </div>
                                <div class="card-body">
...
$(function() {
    ...
    (async function initializeFaceAPI(){
        try{
            ...
            modelsLoaded = true;
            updateStatus('Model berhasil dimuat. Tekan "Start" atau <kbd>S</kbd> untuk mulai scan otomatis.');
            showSpinner(false);
        }catch(e){
    ...
    // Add auto-scan toggle button (you can add this to your HTML)
    // <button id="toggleAutoScan" class="btn btn-info btn-sm">Auto-Scan: ON</button>
    ...
    function stopCamera(){
        ...
        updateStatus('Kamera berhenti. Klik "Start" untuk memulai lagi.');
        clearResult();
    }
...
async function processAutoAttendance(student, confidence) {
    try {
        ...
        if (attendanceResult.success) {
            ...
            // Tambahkan ke riwayat dengan informasi lengkap
            addToAttendanceHistory(
                attendanceResult.student, 
                new Date().toLocaleString('id-ID'), 
                Math.round(confidence * 100), 
                true,
                `${statusText} - ${attendanceResult.status}`
            );
            
            updateStatus(`Absensi ${statusText.toLowerCase()} berhasil untuk ${attendanceResult.student.name}`);
        } else {
            // Jika sudah absen atau error lainnya
            showScanMessage(`${student.name} dikenali (${Math.round(confidence * 100)}% match) - ${attendanceResult.message}`, 'warning');
            
            // Tetap tambahkan ke riwayat tapi dengan status berbeda
            addToAttendanceHistory(
                student, 
                new Date().toLocaleString('id-ID'), 
                Math.round(confidence * 100), 
        ...
        // Tambahkan ke riwayat dengan status error
        addToAttendanceHistory(
            student, 
            new Date().toLocaleString('id-ID'), 
            Math.round(confidence * 100), 
    ...
    function showUserProfile(result){
        ...
        $('#userBirthPlace').text(student.birth_place || '-');
        $('#userBirthDate').text(student.birth_date || '-');
        $('#userAddress').text(student.address || '-');
        $('#userRegistered').text(student.face_registered_at || '-');
    ...
    function showScanMessage(message, type){
        if(type === 'success'){
            ...
            $('#errorText').text(message);
            $('#errorMessage').removeClass('alert-warning alert-danger').addClass('alert-danger').show();
            $('#successMessage').hide();
        }
        $('#scanMessage').show();
    ...
    function addToAttendanceHistory(student, time, confidence, isMatch, attendanceStatus){
        ...
        const studentName = student ? student.name : 'Tidak dikenali';
        const studentNisn = student ? (student.nisn || '-') : '-';
        
        let statusBadge;
        if (!isMatch) {
            statusBadge = '<span class="badge bg-warning">Tidak dikenali</span>';
        } else if (attendanceStatus === 'Berhasil Absen') {
            statusBadge = '<span class="badge bg-success">Absen Berhasil</span>';
        } else if (attendanceStatus && attendanceStatus.includes('sudah')) {
            statusBadge = '<span class="badge bg-info">Sudah Absen</span>';
        } else if (attendanceStatus === 'Error Absensi') {
            statusBadge = '<span class="badge bg-danger">Error Absensi</span>';
        } else {
    ...
    // Initialize
    updateStatus('Sistem Pengenalan Wajah dengan Auto-Absensi Siap. Klik "Start" untuk memulai.');
});
</script>
@endpush